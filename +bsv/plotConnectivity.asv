function [projectionMatrix_array, projectionMatrixCoordinates_ARA] = plotConnectivity(experimentData, allenAtlasPath, inputRegion, numberOfChunks, numberOfPixels, plane, regionOnly, smoothing, colorLimits, color, normalizationInfo, experimentCount)
...
% Handle optional experiment count parameter
if nargin < 12 || isempty(experimentCount)
    experimentCount = []; % Will be auto-detected
end
...
for iChunk = 1:numberOfChunks
    ...
    for iGroup = 1:nGroups
        ...
        if iChunk == numberOfChunks && iGroup == nGroups
            ...
            % Use provided experiment count or try to detect from data
            if ~isempty(experimentCount)
                nExperiments = experimentCount;
            else
