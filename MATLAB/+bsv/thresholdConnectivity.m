...
% Assuming projection_view_bins is a 10x1 cell array
% Each element is a 1x2 cell array
% These nested cells each have 16 values for x bins and y bins
result = zeros(16, 16, 10, 3);

% Loop through each of the 10 AP values
for ap = 1:size(projection_view_bins,1)
    % Extract ML and DV bins for this AP value
    ml_bins = projection_view_bins{ap}{1};
    dv_bins = projection_view_bins{ap}{2};
    
    % Create meshgrid for this AP slice
    [ML, DV] = meshgrid(ml_bins, dv_bins);
    
    % Store the result for this AP slice
    % Each element in this 16x16 slice represents a unique ML-DV-AP coordinate
    result(:,:,ap, 3) = ML;
    result(:,:,ap, 2) = DV;
    result(:,:,ap, 1) = repmat(sliceARAs(ap)*10,size(ML));
end
...
projectionMatrixCoordinates_ARA = result;
end
