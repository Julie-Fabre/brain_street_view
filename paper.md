...
![**Figure 2.** VISam projections to the caudate putamen grouped by the anterior-posterior (AP) location of the *injection site*. Experiments are binned into equal-width AP levels (three here); each row shows the mean projection density for one level, labelled with the mean injection-site AP coordinate in Allen CCF space (µm; larger values are more posterior). This reveals how projection topography in the CP varies with injection AP location. Scale bar: 1 mm.\label{fig:ap_grouped}](images/docs/plot_ap_grouped_VISam_CP.png)

![**Figure 3.** 3D rendering of VISam injection sites (teal dots) overlaid on a transparent volume of the mouse brain, with the caudate putamen highlighted. Dot size reflects injection volume.\label{fig:plot3d}](images/docs/plot_3d_VISam_CP.png)

![**Figure 4.** Interactive upstream projectome (`plot_upstream_projectome`), shown here as a single coronal slice (Allen CCF, 5500 µm). Projections into the caudate putamen from four visual cortical areas are overlaid in distinct colours on the atlas structure boundaries; in the interactive widget an anterior-posterior slider scrolls through the brain, and injection-site dots appear at their own AP levels.\label{fig:projectome}](images/docs/plot_upstream_projectome_CP.png)
