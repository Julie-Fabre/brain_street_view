...
2. **Data retrieval**: `fetch_connectivity_data` downloads projection density volumes for each experiment, caches them locally, and applies user-specified normalization. It supports multiple atlas resolutions and handles the coordinate transformations needed to align data across experiments. Experiments can optionally be grouped by injection-site location along the anterior-posterior, medial-lateral, or dorsal-ventral axis---binned into a chosen number of equal-width levels---or pooled by source region, allowing projection topography to be compared across injection sites (\autoref{fig:ap_grouped}).
3. **Visualization**: `plot_connectivity` generates 2D slice views with region masking, while `plot_connectivity_3d` produces 3D isosurface renderings. `plot_connectivity_multi_region` enables side-by-side comparison of projections from different source regions. `plot_upstream_projectome` provides an interactive coronal viewer, with an anterior-posterior slider to scroll through the brain, that overlays the projections converging on a target region from several source regions at once---each source in its own colour---together with their injection sites (\autoref{fig:projectome}). `threshold_connectivity` applies signal thresholding using multiple statistical methods.
4. **Analysis**: `analyze_cp_subregions` decomposes projection signals into anatomical subdivisions of the caudate putamen and nucleus accumbens, producing a mean projection intensity per subregion---broken down per slice and per group, and as a global per-subregion summary---with optional CSV export (\autoref{fig:subregions}).
...
![**Figure 1.** Interactive upstream projectome (`plot_upstream_projectome`), shown here as a single coronal slice (Allen CCF, 5500 µm). Projections into the caudate putamen from antero-medial visual cortex (VISam) and primary somatosensory cortex, mouth region (SSp-m) are overlaid in distinct colours on the atlas structure boundaries; where projections from both sources overlap the same voxel the colours alpha-blend. In the interactive widget an anterior-posterior slider scrolls through the brain, and injection-site dots appear at their own AP levels.\label{fig:projectome}](images/docs/plot_upstream_projectome_CP.png)

![**Figure 2.** 3D rendering of VISam injection sites (teal dots) overlaid on a transparent volume of the mouse brain, with the caudate putamen highlighted. Dot size reflects injection volume.\label{fig:plot3d}](images/docs/plot_3d_VISam_CP.png)

![**Figure 3.** 2D coronal slices showing projection density from antero-medial visual cortex (VISam) to the caudate putamen (CP), across 10 evenly spaced anterior-posterior levels. Each panel shows the mean projection density within the CP boundary (blue outline), with darker values indicating stronger projections.\label{fig:plot2d}](images/docs/plot_2d_VISam_CP.png)

![**Figure 4.** VISam projections to the caudate putamen grouped by the anterior-posterior (AP) location of the *injection site*. Experiments are binned into equal-width AP levels (three here); each row shows the mean projection density for one level, labelled with the mean injection-site AP coordinate in Allen CCF space (µm; larger values are more posterior). This reveals how projection topography in the CP varies with injection AP location. Scale bar: 1 mm.\label{fig:ap_grouped}](images/docs/plot_ap_grouped_VISam_CP.png)

![**Figure 5.** Quantitative subregion analysis from `analyze_cp_subregions`. Projections from visual cortical areas are decomposed into anatomical subdivisions of the caudate putamen and nucleus accumbens (defined by the Allen v2 atlas), yielding the mean projection fluorescence intensity per subregion and identifying which striatal subregions receive the strongest input.\label{fig:subregions}](images/docs/striatum_subregions.png)
